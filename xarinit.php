...
function tinymce_upgrade($oldversion)
{
    ...
    switch ($oldversion) {
        ...
        case '3.0.4': // version update to reflect updated code, no db changes
            xarModAPIFunc('tinymce','admin','resetdefault',array('resettype'=>'configs'));
        case '3.1.0': // version update to reflect updated code, and stability level for this V 3 series
        case '3.1.1':
            //update to reflect upstream project fixes added to 3.3.9.1
            //plugin fixes (dphighlight)
        case '3.1.2':
            //update to reflect changes for new property validations
        case '3.1.3':
            //upgrade to Unstream tinymce 3.4
        case '3.1.4':
           xarRegisterMask('SubmitTinyMCE','All','tinymce','instance','All','ACCESS_COMMENT');
        case '3.2.0': //current version
            //upgraded to 3.4.2 plus xarigami 1.4.0 only
        case '3.3.0':
            //upgraded to 3.5.1
         case '3.4.0':
            //upgraded to 3.5.8 upstream version
        default:
       break;
       //always reset the configs in case there are new or changed ones
        xarModAPIFunc('tinymce','admin','resetdefault',array('resettype'=>'configs'));
    }
    return true;
}
...
function tinymce_delete()
{
    ...
    // Generate the SQL to drop the table using the API
     $query = xarDBDropTable($tinymcetable);
    try {
          $result = $dbconn->Execute($query);
    } catch (Exception $e) {
        //
